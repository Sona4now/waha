...
export default function QuickActions({ destination }: Props) {
    ...
    try {
      ...
      showToast(
        isFavorite
          ...
          : locale === "en"
            ? `Added ${destination.name} to favorites ❤️`
      ...
      showToast(locale === "en" ? "Something went wrong" : "حدث خطأ", "error");
    }
  }
  ...
  async function handleShare() {
    ...
    try {
      ...
      setShowCopied(true);
      showToast(
        locale === "en" ? "Link copied to clipboard 📋" : "تم نسخ الرابط للحافظة 📋",
        "success"
      );
      setTimeout(() => setShowCopied(false), 2000);
    } catch {
      showToast(locale === "en" ? "Couldn't copy" : "لم يتمكن من النسخ", "error");
    }
  }
  ...
  function openWhatsApp() {
    const text = encodeURIComponent(
      locale === "en"
        ? `Hi, I'm interested in wellness tourism information for ${destination.name}. Can you help me?`
